...
batched tensors. This removes the multiprocessing pool entirely and lets a
single champion generation run thousands of parallel simulations, which is
what "scaling vastly" means on the CPU-bound baseline.

The GPU trainer subclasses :class:`HeadlessTrainer` so it reuses the exact same
map generation, elitist shared-map (1+lambda)-ES loop, adaptive mutation, and
telemetry publishing - only the simulation is different.
"""
...
from training.trainer import HeadlessTrainer
...
class TorchSimulator:
    ...
    All physics runs in float32 for maximum throughput on consumer GPUs; the
    exact tile-resolution mechanics (collision, idle drain, BFS healing) match
    the CPU path so the same champion behaves identically on both backends.
    """
    ...
    def __init__(self, device: torch.device) -> None:
        ...
        self.num_rays: int = max(1, int(config.VISION_RAYS))
        self.max_dist: float = float(config.VISION_MAX_DIST)

        half_arc: float = math.radians(
            float(config.VISION_ARC_ANGLE) / 2.0
        )
        if self.num_rays > 1:
        ...
        self.rad_per_frame: float = (
            math.radians(float(config.TURN_SPEED)) / float(config.FPS)
        )
        self.move_speed: float = float(config.MOVE_SPEED)
        self.radius: float = 0.5 * float(config.PLAYER_RADIUS_RATIO)
    ...
    def sample_vision(
        ...
        Mirrors ``VisionArcSampler.sample_vision_channels_batch`` on the
        device: each ray advances grid-cell boundary by grid-cell boundary
        and reports proximity ``1 - t_entry / max_dist`` at the first wall
    ...
    def compass(
        ...
        """
        Stereo binocular target compass. ``exit_pos`` is (N, 2) in tile units.
        """
    ...
    def kinematics_step(
        ...
        Mirrors CandidateKinematics.step_batch bit-for-bit on the device.
        """
        ...
        move_effort: torch.Tensor = move_effort.reshape(-1)
        turn_effort: torch.Tensor = turn_effort.reshape(-1)
    ...
    def simulate_candidates(
        ...
        """
        Runs every candidate genome in ONE batched tensor job over the shared
        map set. Physics, vision, and collision all run on (C*N) rows so the
        per-step fixed launch cost is paid once for the whole population
    ...
    def _simulate_rows(
        ...
        """
        Shared step-by-step simulation over (R,) rows; ``forward(features)``
        must return ``(move_eff, turn_eff)`` each shaped (R, 1). Returns raw
        per-row CPU lists for state reconstruction.
        """
        n: int = int(wall_grids.shape[0])
        h: int = int(wall_grids.shape[1])
        w: int = int(wall_grids.shape[2])
        ...
        for step in range(1, max_steps + 1):
            ...
            hlt: torch.Tensor = healths - (
                hit.to(self.dtype) * config.HEALTH_COLL_DMG_PER_FRAME
            )
...
def _stack_modules(
    ...
    """
    Stacks per-candidate layer weights into (C, in, out) and biases into
    (C, out) tensors for a batched MLP forward pass.
    """
...
def _build_states(
    ...
    """
    Rebuilds PlayerState objects from the raw per-row CPU lists returned by
    :meth:`TorchSimulator._simulate_rows`.
    """
    (
        xs_cpu,
        ys_cpu,
...
class GpuHeadlessTrainer(HeadlessTrainer):
    """
    Same single-agent evolution as the CPU trainer, but every generation is
    simulated as one big batched tensor job on the GPU.
    """
    ...
    def __init__(
        ...
        self._simulator: TorchSimulator = TorchSimulator(self.device)
    ...
    def _generate_run_maps(self, count: int) -> List[Tuple]:
        """
        Builds the generation's shared map set across the available CPU cores
        instead of serially, since Python-side maze generation dominates the
        GPU per-generation wall time. The next generation's batch is submitted
        to the pool before returning so it overlaps with the caller's
        simulation.
        """
        if count <= 0:
            return []
    ...
    def _submit_map_batch(self, count: int) -> List[Any]:
        """
        Submits ``count`` map-generation tasks with independent seeds and
        returns the futures without waiting on them.
        """
        seeds: List[int] = [
    ...
    def _shutdown_process_pool(self) -> None:
        """
        Shuts down the parallel map pool (and any in-flight prefetch batch) in
        addition to the base pool.
        """
