...
The trainer loads the active profile from brain_library.yaml via the storage
registry, attempts to reload saved .npz champion weights, and persists the
champion upon training completion or new fitness records.
"""
...
def _initialize_worker_priority() -> None:
    """
    Adjusts child process priority natively without external libraries.
    Lowers scheduling priority to Below Normal (Windows) or Nice (Unix/macOS).
    """
    if not getattr(config, "TRAINING_USE_LOW_PRIORITY", True):
        return
...
def _simulate_runs_worker(
    ...
    metabolics: Dict[str, Any] = profile_config["metabolics"]
    kinematics_cfg: Dict[str, Any] = profile_config["kinematics"]
...
def run_training_process(
    ...
    final champion available in shared state until told to stop.
    """
    backend: str = str(
...
class HeadlessTrainer:
    ...
    def __init__(
        ...
        """
        Initializes registry, active profile, champion agent, and persistence.
        """
        ...
        if configured_workers <= 0:
            logical_cores: int = mp.cpu_count() or 1
            configured_workers = max(1, (logical_cores // 2) - 1)
    ...
    def publish_state(self) -> None:
        """
        Writes the current champion genome and profile telemetry into shared
        state consumed by the display runner.
        """
    ...
    def _get_metabolic_params(self) -> Dict[str, Any]:
        """
        Extracts metabolic and sensory parameters from active profile config.
        """
        metabolics: Dict[str, Any] = self.profile_config["metabolics"]
        kinematics: Dict[str, Any] = self.profile_config["kinematics"]
        sensory: Dict[str, Any] = self.profile_config["sensory"]

        return {
            "collision_damage": float(metabolics["collision_damage"]),
            "idle_damage": float(metabolics["idle_damage"]),
            "recovery_ratio": float(metabolics["recovery_ratio"]),
            "move_speed": float(kinematics["move_speed"]),
            "include_bfs_sensor": bool(sensory["include_bfs_sensor"]),
        }
    ...
    def _generate_run_maps(self, count: int) -> List[Tuple]:
        """
        Builds count fresh run tuples (serial on the main thread).
        """
    ...
    def _make_chunks(self, num_runs: int = 0) -> List[List[int]]:
        ...
        for idx in range(num_runs):
            chunks[idx % worker_count].append(idx)
    ...
    def _ensure_process_pool(self) -> None:
        """
        Creates the persistent portable worker pool with low-priority
        scheduling.
        """
    ...
    def _simulate_candidates(
        ...
        fitnesses are directly comparable (paired evaluation).
        """
    ...
    def _update_mutation_scale(self, accepted: bool) -> None:
        """
        Applies the 1/5 success rule and stagnation bumps to mutation scale.
        """
    ...
    def _adapt_run_budget(self, gen_time: float) -> None:
        """
        Runtime self-correction for auto-tuning.
        """
    ...
    def run_training_session(
        ...
        """
        Runs single-agent evolution and auto-saves champion weights to disk.
        """
        print(
